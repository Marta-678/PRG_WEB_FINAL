...
export const getProjectById = async (req, res, next) => {
  try {
    const project = await findProjectInCompany(req.params.id, req.user.company);
    res.status(200).json({ status: 'success', data: { project } });
  } catch (error) {
    next(error);
  }
...
export const deleteProject = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ status: 'success', message: 'Proyecto eliminado correctamente' });
  } catch (error) {
    next(error);
  }
};
...
export const restoreProject = async (req, res, next) => {
  try {
    const project = await findProjectInCompany(req.params.id, req.user.company, { includeDeleted: true });
