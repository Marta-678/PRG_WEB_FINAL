...
import { uploadDeliveryNotePdf } from './storage.service.js';
...
export const generateDeliveryNotePdfBuffer = (deliveryNote, signatureBuffer) =>
  new Promise((resolve, reject) => {
    ...
    doc
      ...
      .text(`Fecha: ${new Date(workDate).toLocaleDateString('es-ES')}`, 50, 73);
    ...
    if (format === 'hours') {
      ...
      if (workers && workers.length > 0) {
        ...
        doc.fontSize(9).fillColor(COLORS.primary).text('Nombre', 60, y + 4).text('Horas', 460, y + 4);
        y += 20;

        workers.forEach((w, i) => {
          doc.rect(50, y, 495, 18).fill(i % 2 === 0 ? '#ffffff' : COLORS.light);
          doc
            .fontSize(9)
        ...
        doc.fontSize(9).fillColor('#ffffff').text('TOTAL', 60, y + 4).text(`${hours || 0} h`, 460, y + 4);
        y += 28;
      }
    } else {
    ...
    if (signatureBuffer) {
      try {
        doc.image(signatureBuffer, 50, y, { width: 200, height: 80, fit: [200, 80] });
        y += 95;
      } catch {
    ...
    doc
      .fontSize(8)
      .fillColor(COLORS.secondary)
      .text(`Generado por BildyApp el ${new Date().toLocaleString('es-ES')}`, 50, 768, {
        align: 'center',
        width: 495,
      });
...
export const generateAndUploadPdf = async (deliveryNote, signatureBuffer) => {
  const buffer = await generateDeliveryNotePdfBuffer(deliveryNote, signatureBuffer);
  const { url } = await uploadDeliveryNotePdf(buffer);
  return url;
};